...
**本地更新版本：`2026.09.02-fast-click-phase2.1` · 快速精确落子 · Windows x64 单 EXE 免安装版**
...
本版按确认的 H5 原型移植暖白 / 墨绿工作台、圆角控件、推荐走法卡片、开关及落子动效，并标注作者“第一少帅”。加入自动执棋判断，以及 F1 急停、严格搜索预算、可解释单步确认、思考并行、区域复用和快速姿态复核等接管优化；不增加浏览器运行时依赖。
...
- 己方落子后会等待游戏画面确认；对手的新盘面必须能从上一确认盘面唯一解释为一枚棋子的一步移动，才会单帧通过并进入下一轮；动画、错识别或多子变化不会通过。
...
- 识别出能唯一解释的对手走子时立即按用户预算搜索。只有确认局面、双方走棋历史、预算和候选数全部一致，才可使用预先计算的结果；候选变化或 F1 急停会取消旧搜索。
...
- 每次落子都会在点击前重新截图、刷新坐标，并验证光标确实到达目标像素和落子结果；落子未生效最多重试三次，仍失败则停止重试并等待盘面安全变化。ONNX 推理会按处理器规模使用最多 6 个工作线程，兼顾识别速度、引擎和游戏响应。
