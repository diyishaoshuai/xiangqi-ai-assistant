...
**当前版本：`2026.08.31-smooth-ui-autoside` · 流畅界面与自动执棋版 · Windows x64 单 EXE 免安装版**

本版按确认的 H5 原型移植暖白 / 墨绿工作台、圆角控件、推荐走法卡片、开关及落子动效，并标注作者“第一少帅”。加入自动执棋判断，以及 F1 急停、严格搜索预算、双帧确认与思考并行、区域复用和逐像素快速复核等接管优化；不增加浏览器运行时依赖。
...
- 思考结束后优先用一张新截图证明盘面完全未变，通过即可开始点击；未通过则完整复核三帧。稳定帧采样间隔缩短至 60 毫秒，遮挡、动画等造成盘面变化时仍须等待安全确认。
- 第一帧识别出可解释的对手走子时，即开始按用户预算搜索，同时继续第二帧确认。只有最终确认的局面、双方走棋历史、预算和候选数全部一致，才可使用预先计算的结果；未确认不点击，候选变化或 F1 急停会取消旧搜索。
- 连续识别优先重新检查上次成功定位的区域，每帧仍运行新的定位模型。已锁定位置出现动画/低置信度格子时先换新截图，不连续对同一坏帧搜索全部区域；持续失败仍会恢复全屏搜索。不修改模型或降低定位、分类校验标准。
- 首次点击使用 100 毫秒两点间隔和 30 毫秒定位停顿，未生效重试回到原来的较慢节奏。发送后 100 毫秒开始确认，不再固定等 450 毫秒；若画面尚未变化，至少等待 600 毫秒才允许判定未生效，避免过早重复点击。
- 用户移动鼠标或切换窗口时会自动暂停，不争抢鼠标；游戏窗口回到前台、用户空闲 1.2 秒且通过上述盘面复核后会自动恢复。
- 思考时间设置为 0.5 秒时，引擎收到的搜索预算就是 500 毫秒；避循环/避连将仅在同一预算内增加候选，不再偷偷加到五秒。屏幕抓取、必要识别、引擎首次初始化及游戏自身动画不属于搜索预算；日志分开记录思考和思考后等待耗时。
...
- 同一局面第二次出现时，助手会在用户设置的思考时间内比较至少 5 条候选，并避开已经导致循环的根着；只有仍能保持胜势的候选才会被提升为新首选着。
...
- 助手会记录己方连续将军次数。连续两手将军后，下一次自动搜索会在用户设置的思考时间内比较至少 12 条候选，不增加时间预算。
...
点击程序中的“运行日志”（旧版“打开日志”），或在资源管理器地址栏输入 `%LOCALAPPDATA%\XiangqiAI\logs`。复现问题后可提交 [Issue](https://github.com/diyishaoshuai/xiangqi-ai-assistant/issues)，附上版本号、Windows 版本、操作步骤、出错时间及对应日志片段。日志可能包含本机路径和棋局信息，公开前请检查、遮盖个人信息；程序本身不会上传日志或截图。
...
本版验证包括全部通过的 127 项源码回归测试和 109 项单 EXE 集成断言；成品通过 14 组内置功能自检，其中包含 21 项界面回归与 74 项自动接管相关测试。新增自动执棋、手动覆盖、低置信度拒绝、新局换边、点击前朝向变化和非阻塞动效检查。接管检查使用模拟 Win32 接口，不会点击真实棋盘；这些结果不等于已在所有游戏客户端或其他电脑上验证。详细范围见[本版发布说明](docs/releases/v2026.08.31-smooth-ui-autoside.md)，旧版见[界面优化版发布说明](docs/releases/v2026.08.31-ui.md)与[免安装版发布说明](docs/releases/v2026.08.31-portable.md)。
