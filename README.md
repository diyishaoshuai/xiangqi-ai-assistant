...
**当前已发布版本：`2026.08.31-ui` · 界面优化版 · Windows x64 单 EXE 免安装版**

[下载 XiangqiAI.exe](https://github.com/diyishaoshuai/xiangqi-ai-assistant/releases/latest/download/XiangqiAI.exe) · [最新 Release](https://github.com/diyishaoshuai/xiangqi-ai-assistant/releases/latest) · [本版更新说明](docs/releases/v2026.08.31-ui.md) · [反馈问题](https://github.com/diyishaoshuai/xiangqi-ai-assistant/issues)
...
1. 双击 `XiangqiAI.exe`。
2. 在游戏中截图，然后点击顶部“粘贴识别”或“导入截图”。程序会自动还原棋子（旧版对应“粘贴并识别”或“导入并识别”）。
3. 在棋盘下方分别选择“我的执棋：执红/执黑”和“当前轮次：红走/黑走”。执黑时棋盘会自动旋转，让黑方位于下方。
...
`2026.08.31-ui` 使用浅色工作台、青绿色重点操作和暖色棋盘，统一字体、间距及操作状态。界面调整不更换象棋引擎、识别模型或 F1 接管流程。

![新版象棋工作台](docs/images/workspace.png)

- 顶部集中放置“粘贴识别”“导入截图”和“自动接管鼠标 F1”；底部保留运行状态、窗口置顶、日志和使用说明入口。
...
- 启动前先正确选择“我的执棋：执红/执黑”和当前“红走/黑走”，并让完整棋盘显示在 Windows 主屏幕上。
...
“粘贴识别”和“导入截图”使用预训练 ONNX 深度模型。第一阶段自动检测棋盘四个外角并完成透视、尺寸归一化；第二阶段一次预测全部 90 个交叉点以及红黑双方十四种棋子，不依赖固定截图分辨率或固定亮度。
...
本版验证包括 65 项单元测试（64 项通过、1 项因缺少原作者本机截图夹具而跳过）和 113 项单 EXE 集成断言。单元测试包含 17 项界面回归及 21 项接管模拟检查；成品也通过 14 组内置功能自检。界面回归覆盖缩放、翻转、调参、FEN、摆子、候选落子、单步推演的跟随、滚动和 F1，并修正旧跟随自检对推演长度的假设。接管检查使用模拟 Win32 接口，不会点击真实棋盘；这些结果不等于已在所有游戏客户端或其他电脑上验证。详细范围见[本版发布说明](docs/releases/v2026.08.31-ui.md)，旧封装版本见[免安装版发布说明](docs/releases/v2026.08.31-portable.md)。
